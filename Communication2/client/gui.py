...
def check_username(username):
    ...
    account_users = client.list_accounts()
    pwd_hash = client.get_password(username)
...
def delete_account():
    ...
    db_accounts = []
    db_user_data = [0, [], [], []]
    status = client.delete_account(login_username.get())
    if not status:
...
def clicked_msg_checkbox(check_var, btn, user, msgId):
    ...
    client.check_message(user, msgId)
...
def filter_recipients(event, row):
    ...
    updated_accounts = client.list_accounts()
